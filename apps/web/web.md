...
| `features/workbench/components/chunk-editor/chunk-editor.test.tsx` | 16 | ChunkEditorWorkbench + ChunkEditModal，Browse/Edit Modal/Persist 三层行为分解：加载→列表、搜索过滤、空状态、错误状态、弹窗预填充、pre-publish vs post-publish 模式差异、按钮禁用逻辑、JSON 容错、保存成功/失败 |
...
- ✅ Vitest 单元/组件测试（store、errors、status、utils、Badge、Card、EmptyState、Alert、BackendGap）— 125 用例全绿
- ✅ Chunk Editor Behavioral Decomposition 测试 — 16 用例，Browse/Edit Modal/Persist 三层覆盖
