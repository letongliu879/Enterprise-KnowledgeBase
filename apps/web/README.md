...
前端**只连接 2 个后端服务**，所有业务数据通过 `workbench-api` 代理，浏览器不直接接触下游服务：

| 服务 | 基础地址 | 认证方式 | 用途 |
|---|---|---|---|
| admin | `NEXT_PUBLIC_ADMIN_API_URL` | Bearer JWT | 集合管理、健康检查、检索配置 |
| workbench-api | `NEXT_PUBLIC_WORKBENCH_API_URL` | Bearer JWT | 上传、复核、任务、文档库、检索代理 |
