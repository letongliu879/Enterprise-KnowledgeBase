...
│  ┌────────────────────────────────────────────────────────┐  │
│  │                    apps/web                            │  │
│  │  /upload /review /retrieval /collections /settings     │  │
│  │  /documents /workbench                                 │  │
│  └────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
           │                 │                   │
...
| 服务 | 语言 | 端口 | 认证 | 职责 | 状态 |
|---|---|---|---|---|---|
| **workbench-api** | Python | 18083 | Bearer JWT | 文档上传、审批工作台、生命周期跟踪 | MVP，7 个下游 API 返回 501 |
| **admin** | Python | 18084 | Bearer JWT | 集合管理、API 密钥、检索配置、运维控制面 | MVP |
| **access** | Java | 18181 | X-API-Key | REST + MCP 双入口、认证、请求翻译、trace | MVP |
| **retrieval** | Java | 18182 | 无 (caller-gated) | 权限感知混合检索、精排、上下文包装 | MVP |
| **indexing** | Python | 18080 | 无 (应用层授权) | 解析、分块、embedding、索引写入与版本管理 | MVP |
...
- **单元测试**：contracts 174 / admin 78 / workbench-api 58 / indexing 53 / access 39 / retrieval 89
- **前端**：Next.js 16，Playwright E2E 18/18 PASS

### 过渡中

- workbench-api 有 7 个下游 API 返回 501（DOWNSTREAM_NOT_IMPLEMENTED）
...
1. 本文（总体架构）
2. `services/intake-pipeline/intake-pipeline.md`（摄入治理详细设计）
3. `services/indexing/indexing.md`（索引构建详细设计）
4. `services/access/access.md`（接入层详细设计）
5. `services/retrieval/retrieval.md`（检索层详细设计）
