...
- 降级策略：若 `indexing` 暂时不可用，允许从本地 `parse_snapshots`、`chunk_registry` 回退读取，保障已入库文档的 ParseSnapshot / chunk 查看

### 6.2.1 Source Preview

- `Source Preview` 与 ParseSnapshot 分离建模
- `Source` tab 只展示原文预览资产，不展示 `preview_text`
- `Parsed text` tab 才展示 ParseSnapshot 的 `preview_text`
- workbench 通过 `GET /workbench/source-files/{id}/preview` / `preview/content` 读取原文预览
- preview 资产 owner 在 `intake-pipeline/conversion-worker`，workbench 只做代理
...
- `GET /workbench/documents/{doc_id}` — 从 SQL projection 查询单文档详情
- `document_state` 是文档列表过滤的 canonical 参数；`status` 仅保留兼容旧前端
- `ProjectionReconciler.reconcile_documents()` 可从本地事实表（`published_documents`、`documents`、`intake_jobs`、`source_files`、`parse_snapshots`、`indexed_documents`、`chunk_registry`）回填不完整的 `workbench_document_projection`
...
GET /workbench/source-files/{id}/preview   # 源文件预览元数据
GET /workbench/source-files/{id}/preview/content  # 源文件预览内容流
...
GET /workbench/documents/{doc_id}         # 单文档详情（读 projection）
