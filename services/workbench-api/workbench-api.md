...
| `workbench-api` | 文档处理人员/业务人员/审批人员 | 上传文档、预览 ParseSnapshot、调 parser 参数沙盒、chunk 确认、审批 | `services/workbench-api` |
| `admin-console` | 平台管理员/运维 | 全局配置、质量评测、运维控制、override | `services/admin` |

## 3. 技术栈

- **单体 FastAPI**（Python）
- **REST**：所有操作（上传、查询、预览、审批决策）
...
- 生成 `source_file_id`，调 `intake-pipeline` 的 `/internal/source-files` 注册
- 本地维护 `workbench_upload_sessions` 聚合状态
- 前端轮询 `GET /workbench/uploads/{id}` 看综合进度
- 下游代理：调 `services/intake-pipeline`
...
1. **发布前编辑**
   ...
   - 审批通过后，由 `services/indexing` 使用 ParseSnapshot + chunk edits materialize 正式索引
...
- `GET /internal/chunk-revisions/{revision_id}` — 查询 revision
- `POST /internal/chunk-revisions/{revision_id}/materialize` — 物化 revision

**retrieval 端点**：
- `POST /internal/cache/purge` — 按 scope 清理检索缓存

### 6.5 审批工作台

- Pending Ticket 列表（支持按 collection、visibility、routing_recommendation 过滤）
- 单票详情：ParseSnapshot 预览 + agent review 结果 + approval audit log
- AgentReview 展示内容包括 decision、quality findings、risk flags、evidence anchors、model/version/prompt hash、suggested fixes、degraded/failure reason
...
workbench 本地维护 `workbench_upload_sessions` 表，把多下游状态串成一条线：
...
`workbench_upload_sessions.status` 是 UI 聚合状态，不是流程事实源。它必须能从 source_file、intake_job、ParseSnapshot、approval_ticket、published_document 重新计算；下游事实变化时，以 owner 服务状态为准。
...
|------|---------|--------------|------|
| 文件上传 | `intake-pipeline` | `POST /internal/source-files` | **已实现** |
| 上传任务状态 | `intake-pipeline` | `GET /internal/intake-jobs/{id}` | **已实现** |
| 源文件详情 | `intake-pipeline` | `GET /internal/source-files/{id}` | **已实现** |
| 审批票列表 | `approval-service` | `GET /internal/tickets` | **已实现** |
| 单票详情 | `approval-service` | `GET /internal/tickets/{id}` | **已实现** |
| AgentReview artifact | `approval-service` | `GET /internal/tickets/{id}/agent-review` | **已实现**（stub） |
| 审批决策 | `approval-service` | `POST /internal/tickets/{id}/decide` | **已实现** |
| effective parser profile 列表 | `services/admin` + `services/indexing` | `GET /internal/collections/{id}/parser-profiles` + indexing validate/canonical view | 待补齐 |
| ParseSnapshot 查看 | `services/indexing` | `GET /internal/parse-snapshots/{id}` | **已有** |
| ParseSnapshot chunk preview | `services/indexing` | `GET /internal/parse-snapshots/{id}/chunks` | 待补齐 |
| 沙盒预览触发 | `services/indexing` | `POST /internal/parse-previews` | **已有** |
| chunk 查询 | `services/indexing` | `GET /internal/chunks` | **已有** |
| 发布前 chunk edit 提交 | `services/indexing` | `POST /internal/chunks/{id}/revisions` | **已实现**（通过 chunk_edit submit 调用） |
| 发布后 chunk revision 创建 | `services/indexing` | `POST /internal/chunks/{id}/revisions` | **已实现** |
| chunk revision materialize | `services/indexing` | `POST /internal/chunk-revisions/{id}/materialize` | **已实现** |
| retrieval cache purge | `services/retrieval` | `POST /internal/cache/purge` | **已实现** |
| collection 权限/默认绑定 | `services/admin` | `GET /internal/collections/{id}` | 待补齐 |
...
|------|------------|----------------|--------|
| `Collection` | `services/admin` | 只读选择、按权限使用 | admin internal API |
| `ParserProfile` | 控制面 `services/admin`，运行时 `services/indexing` | 只读选择、per-document override | admin internal API + indexing preview API |
...
GET /workbench/chunks/{id}
PATCH /workbench/chunks/{id}                    # 发布后编辑，内部创建 indexing ChunkRevision
POST  /workbench/chunks/{id}/hide               # 发布后隐藏，内部走 indexing
GET   /workbench/chunks/{id}/revisions
...
# 审批工作台
GET    /workbench/tickets                  # Pending Ticket 列表
GET    /workbench/tickets/{id}             # 单票详情
GET    /workbench/tickets/{id}/agent-review # AgentReview 结果与证据
POST   /workbench/tickets/{id}/decide      # Approve / Reject / Return
GET    /workbench/tickets/{id}/audit-log   # 审批历史

# 任务聚合
GET /workbench/tasks                       # 当前用户的任务列表（聚合视图）
GET /workbench/tasks/{upload_id}           # 单任务完整链路
...
- ticket 列表和详情来自 approval-service
- AgentReview artifact 来自 intake/approval artifact refs
...
- Upload integration：workbench -> intake，返回 source_file_id/intake_job_id，并能轮询 owner 状态
...
- `services/workbench-api/src/workbench_api/downstream_clients/`：async httpx 客户端（indexing、intake、approval、admin）
- `services/workbench-api/src/workbench_api/auth/routes.py`：`GET /workbench/auth/me`
- `services/workbench-api/src/workbench_api/upload_sessions/`：上传会话 CRUD + intake 代理
...
- `services/workbench-api/src/workbench_api/tickets/`：ticket 列表/详情/决策代理 + AgentReview 展示
- `services/workbench-api/src/workbench_api/chunk_edits/`：发布前 chunk edit CRUD（本地表）
- `services/workbench-api/src/workbench_api/task_projection/`：任务聚合视图

**测试覆盖** — 已完成

- `services/workbench-api/tests/`：12 个测试文件，57 个测试用例
- `packages/contracts/tests/test_schema_validation.py`：扩展 workbench schema 验证 + wire drift guard
- 所有 gate 当前通过：contracts（169 passed）、workbench-api（57 passed）
...
| 功能 | 下游服务 | 需要的内部 API |
|------|---------|--------------|
| collection 权限/默认绑定 | `services/admin` | `GET /internal/collections/{id}` |
| ParseSnapshot chunk preview | `services/indexing` | `GET /internal/parse-snapshots/{id}/chunks` |
| 发布前 chunk edit 提交 | `services/indexing` | `POST /internal/parse-snapshots/{id}/chunk-edits` |
| 发布后 chunk revision 创建 | `services/indexing` | `POST /internal/chunks/{id}/revisions` |
