...
`intake-pipeline` 是 Reality-RAG 的企业知识摄入控制面。它负责把外部文件安全、可审计、可恢复地转化为可检索、可治理、可发布的知识文档。

与其他文档的关系：

- 总入口：`docs/上游机制移植总览.md`
- 顶层边界：`docs/最终主线.md`
- 最终态契约：`docs/实现计划.md`
...
- 如接入 RAGFlow 知识库工作台，它只负责解析/分块预览与人工校验，不拥有文档治理真相。
...
`intake-orchestrator` 是唯一拥有 `intake_job_state` 的组件。

其他组件只能做两件事：

- 接收 orchestrator 派发的任务
- 提交阶段结果或治理决策

其他组件不得直接推进全局摄入状态，不得自行决定全链路下一步。
...
如果存在 RAGFlow workbench，对应职责也必须降格理解：

| 组件 | 只负责 | 不负责 |
|------|--------|--------|
| RAGFlow workbench | ParseSnapshot 展示、chunk 预览、参数调试、人工确认 | collection 治理、文档权限、审批结论、发布状态 |
...
`auto_approve` 是 approval-service 产生的 system decision。

自动入库不生成 PENDING 人工工单，但必须生成：
...
approval-service 不写资产、不写索引、不直接持久化最终文档。

approval-service 只产生 `ApprovalDecided`。实际发布由 publishing-worker 执行。
...
| `user_id` | string | nullable for system source |
...
| `last_chunk_at` | timestamp | nullable |
| `completed_at` | timestamp | nullable |

`expected_size` 和 `expected_sha256` 在客户端已知时必须提供；流式上传或未知大小上传可为空，但 `/complete` 时 document-service 必须计算服务端 SHA-256、最终大小，并写回会话与 object metadata。若客户端提供过 expected 值，complete 时必须严格比对。
...
| `sanitized_name` | string | not null |
...
- `(content_hash, collection_id)` partial unique where state in (`UPLOADING`, `UPLOADED`, `SCANNING`, `READY`, `CLAIMED`, `CONSUMED`)
...
| `raw_result_ref` | string | nullable |
...
| `similarity_hints` | jsonb | not null |
| `version_conflict` | jsonb | nullable |
| `converter_name` | string | not null |
| `converter_version` | string | not null |
| `cleaning_version` | string | not null |
| `schema_version` | string | not null |
| `created_at` | timestamp | not null |

唯一约束：

- `(intake_job_id, preliminary_doc_id)` unique

复用规则：

- ConversionStage、CleaningStage、QualityStage 可按 `content_hash + converter_version + cleaning_version + schema_version` 复用。
- SimilarityCheck、VersionCheck 必须按 `collection_id` 查询，不能跨 collection 复用决策结果。

### 6.4 agent-review-worker 表
...
| `schema_version` | string | not null |
| `pii_report` | jsonb | not null |
| `sanitization_spans` | jsonb | not null |
| `visibility_check` | jsonb | not null |
...
- `(canonical_hash, visibility, model_provider, model_name, model_version, prompt_version, schema_version)`

该唯一约束只用于成功 review 结果复用。`review_status=failed/degraded` 的失败尝试不得阻止后续重试；失败细节应写入 stage_attempts 和 telemetry，或以不参与复用的记录保存。

### 6.5 approval-service 表
...
最终文档记录归 publishing domain 所有。document-management API 只能通过 publishing domain 的管理接口或授权命令修改 lifecycle state，不得直接绕过审计写表。
...
### 6.7 indexing-service 表

#### `index_build_jobs`

indexing-service 独立部署时必须拥有自己的任务表和 outbox。publishing-worker 不直接写 vector index 内部状态，只通过 index command 或事件契约驱动 indexing-service。

| 字段 | 类型 | 约束 |
|------|------|------|
| `index_build_job_id` | string | PK |
| `publish_id` | string | nullable |
| `reindex_job_id` | string | nullable |
| `final_doc_id` | string | not null |
| `collection_id` | string | not null |
| `visibility` | enum | INTERNAL / EXTERNAL |
| `target_index_version` | string | not null |
| `sanitized_md_ref` | string | not null |
| `metadata_ref` | string | not null |
| `chunker_version` | string | not null |
...
- `(final_doc_id, target_index_version, state)` partial unique where state in (`queued`, `chunking`, `embedding`, `upserting`, `activating`)。
- 成功后由 indexing-service outbox 发送 `IndexReady`，表示 document index revision 已准备好。
...
- `(final_doc_id, index_version)` unique
...
| `schema_version` | string | not null |
| `trace_id` | string | not null |
| `intake_job_id` | string | nullable |
| `source_file_id` | string | nullable |
| `collection_id` | string | nullable |
| `visibility` | enum | INTERNAL / EXTERNAL / null |
| `stage_name` | string | nullable |
| `stage_task_id` | string | nullable |
| `ticket_id` | string | nullable |
| `final_doc_id` | string | nullable |
| `component` | string | not null |
| `component_version` | string | not null |
| `status` | enum | started / succeeded / failed / degraded / skipped |
| `duration_ms` | int | nullable |
| `error_code` | string | nullable |
| `retry_count` | int | default 0 |
| `attributes_json` | jsonb | sanitized attributes only |

#### `llm_call_log`

每次模型调用记录一行。只记录调用元数据、hash、token、耗时、解析结果，不记录 prompt/response 明文。

| 字段 | 类型 | 约束 |
|------|------|------|
| `llm_call_id` | string | PK |
| `trace_id` | string | not null |
| `intake_job_id` | string | not null |
| `stage_task_id` | string | not null |
| `review_id` | string | nullable |
...
| `status` | enum | succeeded / failed / timeout / rate_limited / schema_invalid |
| `error_code` | string | nullable |
| `retry_count` | int | default 0 |
| `json_parse_success` | bool | not null |
| `schema_validation_success` | bool | not null |
| `redaction_before_send` | bool | not null |
| `external_model_used` | bool | not null |
| `created_at` | timestamp | not null |

#### `review_quality_feedback`

关联 agent-review 输出与 approval-service 决策，用于评估规则、模型、prompt 的效果。
...
| `routing_recommendation` | enum | auto_approve / require_approval |
| `review_status` | enum | succeeded / degraded / failed |
| `pii_count_by_type` | jsonb | counts only |
| `pii_count_by_severity` | jsonb | counts only |
| `visibility_conflict` | bool | not null |
| `visibility_conflict_type` | string | nullable |
| `approval_decision` | enum | approve / reject / return / expire |
| `auto_approved` | bool | not null |
| `manual_override` | bool | not null |
| `return_target_stage` | string | nullable |
| `return_reason_code` | string | nullable |
| `approver_changed_tags` | bool | nullable |
| `approved_after_review_failure` | bool | not null |
| `created_at` | timestamp | not null |

#### `llm_cost_daily`

按天聚合 LLM 成本和稳定性。该表可由 `llm_call_log` 离线生成。

| 字段 | 类型 | 约束 |
|------|------|------|
| `date` | date | PK part |
| `provider` | string | PK part |
| `model_name` | string | PK part |
| `model_version` | string | PK part |
| `prompt_version` | string | PK part |
| `collection_id` | string | PK part |
| `visibility` | enum | PK part |
...
producer：publishing domain（由 document-management API 命令或新版本发布流程触发）
...
1. 用户上传文件到 document-service，必须指定 `collection_ids` 和 `visibility`
2. document-service 写临时对象，校验 SHA-256
3. document-service 创建 `object_blob` 和 `source_file`
4. document-service 扫描恶意文件，通过后 state = READY
5. document-service outbox 发送 `FileReady`
6. intake-orchestrator 幂等创建 `intake_job`
7. orchestrator claim source file，state = CLAIMED；claim 失败则取消新建 job，并返回已有 source/job 状态
8. orchestrator 创建 conversion stage task
9. conversion-worker 执行转换、清洗、质量、相似度、版本线索
10. conversion-worker 提交 `StageCompleted`
11. orchestrator 校验 conversion result，通过后调用 document-service `mark-consumed` 内部 API，将 source file 标记为 CONSUMED
12. orchestrator 推进到 review stage
13. agent-review-worker 执行规则 DLP 和 LLM 风险审核
14. agent-review-worker 持久化 review result，提交 `StageCompleted`
15. orchestrator 生成 `ApprovalRequested`
16. approval-service 判断满足 auto approve
17. approval-service 创建 `approval_ticket`，state = SYSTEM_DECIDED
18. approval-service 生成 `final_doc_id`、规则标签、审计日志
19. approval-service 发送 `ApprovalDecided`
20. orchestrator 创建 publish task
21. publishing-worker 写 asset、persist document，并请求 indexing-service 构建索引
22. publishing-worker 发送 `PublishCompleted`
23. orchestrator 将 job 标记为 PUBLISHED
24. orchestrator 调用 document-service `mark-cleanable` 内部 API，将 source file 标记为 CLEANABLE，document-service GC 再判断 object_ref 是否可删
...
- 质量 C/D
- 版本冲突
- review degraded/failed
- collection policy 要求人工审批

流程：

1. approval-service 收到 `ApprovalRequested`
2. 判断不能 system approve
3. 创建 `approval_ticket`，state = PENDING
4. 发送 `ApprovalPending`
5. orchestrator 将 job state 置为 AWAITING_APPROVAL
6. 审批人操作 approve/reject/return
7. approval-service 追加审计日志
8. approval-service 发送 `ApprovalDecided`
9. orchestrator 根据 decision 推进：
   - approve -> publish
   - reject -> REJECTED
   - return -> 指定 stage queued
...
### 14.5 RequestIndexBuild / WaitIndexReady
...
本章定义对前端、CLI、Admin、运维工具暴露的 API。worker 不直接暴露管理 API。所有管理 API 由对应 service 或 admin-api 聚合层提供。
...
### 16.5 document-management API

已发布文档的管理 API 不由 publishing-worker 暴露。由 document-management API 或 admin-api 聚合层暴露。

document-management API 是命令入口，不直接写 `published_documents`。所有 lifecycle 写入必须进入 publishing domain：

1. document-management API 校验调用方权限、参数和 `Idempotency-Key`
...
- `admin-api:/admin/manual-review/*` 如果仍存在，只能标记为临时兼容路径。
...
### 17.1 published_document_state

owner：publishing domain。document-management API 是对外管理入口，不直接拥有状态表；它通过 publishing domain 的受控命令变更 lifecycle state。
...
本章用于把当前 ingestion-worker 代码映射到目标边界。目标架构不因现有代码妥协；允许同进程部署，但代码边界必须按目标契约切开。
...
### 19.3 IngestionJob 迁移

现有 `IngestionJob` 映射为新模型：
...
- 已完成旧 job 保留只读历史
- 新摄入只写新 `intake_jobs`
- 如需查询统一视图，由 admin-api 聚合旧 `IngestionJob` 和新 `intake_jobs`

## 20. 重复上传语义

### 20.1 duplicate handling

当 `(content_hash, collection_id)` 命中已有记录时，必须先区分已有记录是否仍是 active source file。active source file 不能创建第二条 active 记录；non-active 历史记录不阻止重新提交。

| 已有状态来源 | 已有状态 | 新上传行为 |
|--------------|----------|------------|
| source_file | READY 且无 active job | 返回已有 `source_file_id`，`intake_job_id = null`，提示等待 FileReady 消费或由后台自动触发 |
| source_file/job | READY 且已有 active job / CLAIMED / CONSUMED | 返回已有 `source_file_id` 和 `intake_job_id`，不创建新 active source_file |
| intake_job | PROCESSING / REVIEW / APPROVAL | 返回已有 job 状态 |
| published_document | PUBLISHED 且 `source_content_hash` 相同 | 返回已有 `final_doc_id`，不重复摄入 |
| intake_job | REJECTED | 允许按策略重新提交，但必须创建新的 `source_file_id` 和 `intake_job_id`，并保留旧拒绝审计 |
| intake_job | FAILED | 技术失败可 retry 原 job；如果重新上传，创建新 source_file 并关联同 object_id |
| intake_job | CANCELLED | 不允许直接复用原 `source_file_id` 创建新 job；如需继续处理，由运维 retry 原 job 或由用户重新上传生成新 `source_file_id` |
| source_file | CLEANED | 可创建新 source_file，复用 object_id 或重新写对象 |

### 20.2 多 collection 上传
...
| 阶段 | started | succeeded | failed/degraded |
|------|---------|-----------|-----------------|
...
| similarity | - | `similarity_checked` | - |
| version | - | `version_checked` | - |
...
| publish | `publish_started` | `publish_completed` | `publish_failed` |
| publish substage | - | `asset_written` / `document_persisted` / `index_upserted` | substage failed |
| job | - | `intake_job_completed` | `intake_job_failed` / `intake_job_cancelled` |

### 25.6 阶段事件推荐 attributes

| 事件 | attributes |
|------|------------|
| `upload_completed` | `file_size_bucket`, `source`, `chunked`, `content_hash_prefix` |
| `scan_completed` | `engine`, `engine_version`, `verdict` |
| `conversion_completed` | `file_type`, `converter_name`, `converter_version`, `canonical_size_bucket` |
| `quality_scored` | `quality_grade`, `completeness_bucket`, `density_bucket`, `structure_bucket`, `noise_bucket` |
| `similarity_checked` | `high_similarity_count`, `related_count`, `max_similarity_bucket` |
| `version_checked` | `has_version_conflict` |
| `review_completed` | `review_status`, `routing_recommendation`, `pii_count_total`, `visibility_conflict` |
| `approval_decided` | `decision`, `auto_approved`, `manual_override`, `return_target_stage` |
| `publish_completed` | `chunk_count_bucket`, `index_version`, `embedding_model_version` |
| `asset_written` | `asset_count`, `asset_bytes_bucket`, `payload_hash` |
| `document_persisted` | `final_doc_id`, `published_document_state`, `payload_hash` |
| `index_upserted` | `index_version`, `chunk_count_bucket`, `embedding_model_version`, `index_latency_bucket` |

### 25.7 LLM 调用埋点

每次外部或私有模型调用必须写 `llm_call_log`，并产生 trace span。
...
agent-review-worker 的输出必须与 approval-service 的最终决策关联，形成 `review_quality_feedback`。
...
- `review_status`
- `pii_count_by_type`
- `pii_count_by_severity`
- `visibility_conflict`
- `visibility_conflict_type`
- `approval_decision`
- `auto_approved`
- `manual_override`
- `return_target_stage`
- `return_reason_code`
- `approver_changed_tags`
- `approved_after_review_failure`

质量闭环必须能回答：

- 哪个 `prompt_version` 的人工打回率最高
- 哪类 PII 误报最多
- 哪些 collection 的人工覆盖率最高
- review degraded 后被人工 approve 的比例
- EXTERNAL 文档 visibility conflict 的命中质量
- quality grade 与人工拒绝/打回之间的关系
...
每个 intake job 必须形成单条 trace。关键 span：

- `document.upload`
- `document.scan`
- `orchestrator.create_job`
- `orchestrator.schedule_stage`
- `conversion.run`
- `agent_review.run`
- `agent_review.llm_call`
- `approval.decide`
- `publishing.asset_write`
- `publishing.persist_document`
- `publishing.upsert_index`

trace attribute 不得包含敏感明文。单个 trace 至少可定位：

- 卡在哪个 stage
- 哪次 retry 成功或失败
- 哪个 LLM provider/model 出错
- publish 哪个子阶段失败

### 25.11.1 Run Trace / Artifact Model
...
- 一个 `source_file_id` 最多一个 `intake_job_id`
- 一个 `intake_job_id` 最多一个 `final_doc_id`
- 每次重新进入审批必须创建新的 `ticket_id` 和递增的 `approval_round`
- `stage_results` 只保存成功产物，失败尝试只能写 `stage_attempts`、telemetry 或非复用失败记录
- 一个 `final_doc_id` 只能由 approval-service 生成
- 一个 `final_doc_id` 发布成功后不可变
- publishing-worker 必须按 `final_doc_id` 幂等 upsert
- reindex 必须使用 `reindex_jobs`，不得创建新的 `publish_jobs`
- published document lifecycle 可变，但必须通过 document-management API 命令进入 publishing domain，并追加审计记录
- RETRACTED 文档不得被 retrieval-service 返回
- index_version 表示检索索引契约版本，不等于文档版本
- chunking、embedding、vector index upsert 的 owner 是 indexing-service
- `published_documents` 是发布事实源；`documents` / `document_policies` 只能是兼容读模型或旧投影
- intake 文档不得定义 indexing/retrieval 内部 schema，只能定义发布输出和事件 seam
- publishing-worker 不直接暴露管理 API
- 审批审计只追加，不覆盖
- worker 不推进全局 job 状态
- outbox 事件可重复，消费必须幂等
- source file 状态不等于 document 发布状态
- auto approve 必须产生审批审计
- 普通 telemetry 不得保存文档正文、PII 原值、完整 prompt 明文、完整 LLM response 明文
- 每次 LLM 调用必须记录 model_version、prompt_version、request_hash、response_hash、token、latency、schema validation 结果
- review 质量反馈必须关联 agent-review 输出与 approval 决策
- 跨 service owner 不得直接写表，必须通过 API/event
- 每个配置变更必须有 config_version 和审计记录

## 29. 一句话

`intake-pipeline` 是企业知识进入 Reality-RAG 的控制面。document-service 管原始文件，intake-orchestrator 管全局流程，worker 只执行幂等阶段，approval-service 只做治理决策，publishing-worker 只做幂等发布。所有状态、事件、审计、重试、发布都围绕这个边界设计。
