"""Adapters between StageContext (legacy mutable context) and stage schemas.

StageContext is retained as an adapter layer but is no longer the
long-term contract.  All new orchestration code should work with
schema objects; StageContext is only for backward compatibility
with the existing pipeline.py runner.
"""
...
# ──────────────────────────────────────────────────────────────────────────────
#  Conversion stage adapters
# ──────────────────────────────────────────────────────────────────────────────


def ctx_to_conversion_input(ctx: StageContext) -> ConversionStageInput:
    """Build ConversionStageInput from StageContext."""
...
def conversion_output_to_ctx(
    output: ConversionStageOutput, ctx: StageContext
) -> StageContext:
    """Merge ConversionStageOutput back into StageContext (legacy compat)."""
...
def ctx_to_conversion_output(ctx: StageContext) -> ConversionStageOutput:
    """Build ConversionStageOutput from a StageContext after conversion stages ran."""
    inp = ctx_to_conversion_input(ctx)
    input_hash = compute_input_hash(inp)

    # Build version_conflict from current context
    version_conflict = None
    if ctx.version > 1 and ctx.document_repo is not None:
        # [TRANSITIONAL] Current code auto-archived; we reconstruct conflict info
...
# ──────────────────────────────────────────────────────────────────────────────
#  Review stage adapters
# ──────────────────────────────────────────────────────────────────────────────


def ctx_to_review_input(ctx: StageContext) -> ReviewStageInput:
    """Build ReviewStageInput from StageContext."""
    result = ctx.result
...
def review_output_to_ctx(
    output: ReviewStageOutput, ctx: StageContext
) -> StageContext:
    """Merge ReviewStageOutput back into StageContext."""
...
def ctx_to_review_output(ctx: StageContext) -> ReviewStageOutput:
    """Build ReviewStageOutput from a StageContext after review stage ran."""
...
# ──────────────────────────────────────────────────────────────────────────────
#  Publishing stage adapters
# ──────────────────────────────────────────────────────────────────────────────


def ctx_to_publishing_input(ctx: StageContext) -> PublishingStageInput:
    """Build PublishingStageInput from StageContext.

    [TRANSITIONAL] publish_status currently comes from DecisionStage inside
    the worker.  Target: comes from approval-service ticket.
    """
...
def publishing_output_to_ctx(
    output: PublishingStageOutput, ctx: StageContext
) -> StageContext:
    """Merge PublishingStageOutput back into StageContext."""
...
def ctx_to_publishing_output(ctx: StageContext) -> PublishingStageOutput:
    """Build PublishingStageOutput from a StageContext after publishing stages ran."""
