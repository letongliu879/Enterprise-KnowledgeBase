"""Agent review result cache for ingestion-worker.

R1-002: Cache AgentReview results to avoid repeated DeepSeek LLM calls.
- Cache key is conservative: content hash + quality hash + collection context + model.
- Redis-backed in production, in-memory fallback in dev/test.
- Fail-open: cache failure never blocks ingestion; falls back to real LLM review.
- TTL: approve=24h, reject/quarantine/request_changes=permanent (key-based invalidation).
"""
...
# Schema version bumps when prompt templates or AgentReview schema change materially.
...
def build_cache_key(
    *,
    ...
    """Build a conservative cache key that invalidates on any material change.

    Key segments:
      - schema_version: bumps when prompts or AgentReview schema change
      - content_hash:   sha256(canonical_content)
      - quality_hash:   sha256(quality_report JSON)
      - context_hash:   sha256(collection_id + authority_level)
...
def _ttl_for_review(review: AgentReview) -> int | None:
    """Return TTL in seconds, or None for permanent caching.

    approve           -> 24h
    reject            -> permanent
    quarantine        -> permanent
    request_changes   -> permanent
    """
    raw = review.decision
...
class _InMemoryEntry:
    review: AgentReview
    expires_at: float | None  # None means permanent


class InMemoryAgentReviewCache:
    """Fail-safe in-memory cache for dev/test."""
...
class RedisAgentReviewCache:
    """Redis-backed cache with fail-open semantics on connection errors."""
    ...
    def get(self, key: str) -> AgentReview | None:
        try:
            ...
            # Fail open: treat any Redis error as cache miss
            return None
        if not payload:
            return None
        try:
    ...
    def set(self, key: str, review: AgentReview, ttl_seconds: int | None = None) -> None:
        try:
            ...
            # Fail open: silently drop cache write
            pass
