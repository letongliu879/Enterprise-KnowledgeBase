...
- `Source Preview`（原文预览资产）不是 ParseSnapshot，也不是 `canonical_md`
- `Source Preview` 的正式 owner 应落在 `conversion-worker`
- `document-service` 继续只拥有 `source_file` 生命周期与原始对象引用；它不负责决定 preview 的内容语义
- `workbench-api` 只读取/代理 source preview 元数据或内容，不承担 Office/页面转换
- `indexing` 不负责 source preview；`preview_text` 属于解析结果，用于 Parsed text，不得冒充原文件预览
- 原生可预览格式（`pdf/image/html/text`）可由 `document-service` 直接暴露原对象；非原生格式（`docx/pptx/xlsx/...`）必须委托 `conversion-worker` 生成 preview 资产
- preview 资产优先按 `source_file_id` 落盘缓存；若缓存缺失，则由 `conversion-worker` 即时补生成，而不是让前端兜底
- `document-service` 对外保持 `/internal/source-files/{id}/preview*` 稳定入口，但内部 owner 必须尊重 `conversion-worker`

推荐 preview 资产类型：
- `preview_kind = pdf | image | html | text | unsupported`
...
- `image`：直接原文件预览
- `html`：直接 HTML 预览
- `txt/md`：text 或 HTML 预览
- `docx/pptx`：生成 page-based preview（优先 PDF）
- `xlsx/csv`：优先 HTML table preview；如需统一分页载体可额外生成 PDF
