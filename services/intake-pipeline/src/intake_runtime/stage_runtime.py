...
def _run_review_core(session, job, source_file, ctx, task, attempt, pipeline, conv_row) -> None:
    ...
    artifact_ref = build_review_artifact_path(job.collection_id, job.intake_job_id)
    artifact["agent_review_ref"] = artifact_ref
    write_json_asset(artifact_ref, artifact)
...
def _build_review_artifact(*, job, task, attempt, summary: dict[str, Any]) -> dict[str, Any]:
    review_context = summary.get("review_context", {}) if isinstance(summary.get("review_context"), dict) else {}
    artifact_metadata = (
        review_context.get("artifact_metadata", {})
        if isinstance(review_context.get("artifact_metadata"), dict)
        else {}
    )
    agent_review = summary.get("agent_review", {}) if isinstance(summary.get("agent_review"), dict) else {}
    return {
        "review_run_id": attempt.stage_attempt_id,
        "stage_task_id": task.stage_task_id,
        "intake_job_id": job.intake_job_id,
        "source_file_id": str(artifact_metadata.get("source_file_id") or job.source_file_id or ""),
        "parse_snapshot_id": str(artifact_metadata.get("parse_snapshot_id") or ""),
        "artifact_version": "v1",
...
def run_publishing(session, intake_job_id: str, *, persist_fn=None) -> None:
    from reality_rag_persistence.models import StageResultModel
