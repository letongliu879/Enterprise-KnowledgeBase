"""FastAPI dependencies for admin service."""
...
class CurrentUser:
    def __init__(self, user_id: str, email: str, roles: list[str], tenant_id: str = ""):
    ...
    def can_access_collection(self, collection_id: str) -> bool:
        return True  # TODO: implement collection-level ACL
...
def require_auth(credentials: HTTPAuthorizationCredentials | None = Depends(security)) -> CurrentUser:
    ...
    try:
        ...
        return CurrentUser(user_id=user_id, email=email, roles=roles, tenant_id=tenant_id)
    except JWTError:
