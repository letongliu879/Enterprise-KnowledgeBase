...
- **本地配置**：`alert_rules`、`eval_datasets`、`bad_cases` 等是 admin 本地数据模型
...
- **单体 FastAPI**（Python）
- **REST**：控制操作（retry、cancel、approve、archive、配置 CRUD）
- **GraphQL**：审计查询聚合（trace timeline、跨表关联、分页过滤）
- **鉴权**：query 前置过滤器（数据隔离）+ resolver 层敏感字段控制
- **认证**：`admin_users` 表 + pbkdf2_sha256 密码哈希 + HS256 JWT（可配置 issuer/audience 校验）；smoke/test mode 使用 `smoke-test-secret`，production mode 通过 `ADMIN_JWT_ISSUER`/`ADMIN_JWT_AUDIENCE` + `AUTH_MODE=production` 关闭默认 secret
...
`services/admin` 应按模块化控制面实现，不要做成单层杂糅的 routes/repository 容器。建议最少拆成：

- `identity`：`admin_users`、`admin_sessions`、JWT、角色与租户过滤
- `collection_catalog`：Collection、生命周期、默认绑定、访问策略
- `profile_registry`：ParserProfile、RetrievalProfile、API Key 控制面
- `ops_control`：override、reindex、archive、retract、cache purge
- `observability`：trace timeline、audit、metrics、eval、bad case

护栏规则：

- published profile 不能原地改，只能新 version / new publish
- collection 默认绑定必须版本化，历史任务可回放
- GraphQL 只读 read model / projection，不直接驱动控制动作
- 所有控制动作必须带 `trace_id`、`idempotency_key`、`actor`
...
1. **鉴权**：检查 `admin_user` 的角色权限
2. **审计**：写 `ops_audit_log`（actor、action、target、before_state、after_state、reason）
3. **执行**：控制面对象写 admin 本地表；运行时对象调 owner 服务内部 API，绝不直接操作下游表
...
### 5.2 分块策略工作台（Parser Profile 模板）

- Parser Profile CRUD：定义 `naive`、`presentation`、`paper`、`qa` 等 profile 的名称、版本、默认参数和可用范围
- `services/admin` 是 Parser Profile 控制面 owner，负责创建、编辑、发布、停用和 collection 默认绑定
- `services/indexing` 是 Parser Profile 运行时 owner，负责校验 profile 是否可执行、解析语义、ParseSnapshot 生成和 materialization
- **生效发布流程**：
  1. admin 创建/编辑 profile（状态为 `draft`）
  2. 发布时，admin 调用 `services/indexing` 的 `POST /internal/parser-profiles/validate`
  3. indexing 返回 `canonical_config`、`profile_hash`、`validator_version`、`warnings`
  4. 若 `valid=false`，admin 拒绝发布，写 `ops_audit_log`（`after_state=rejected`），返回 409
  5. 若 `valid=true`，admin 将 `runtime_canonical_config`、`profile_hash`、`validator_version`、`warnings` 写入 profile 记录，状态变为 `published`
  6. 对已 `published` profile 再次发布时，自动创建新 version（旧版本 `retired`）
- published ParserProfile 采用不可变版本；修改已发布配置时只能生成新 version，不允许原地覆盖旧 version
- Collection 绑定：哪个 collection 默认用哪个 parser profile；绑定关系由 admin 持有，profile 可执行性由 indexing 校验
- **注意**：沙盒预览（选 profile 试跑单份文档、对比 chunk 结果）在 `workbench-api`
- admin 只管理全局模板和绑定关系，不直接做沙盒预览

### 5.3 Chunk 人工干预（运维兜底）

- 按 `doc_id` + `index_version` 查看 chunk 列表（adapter 映射到底层 `final_doc_id`）
- 隐藏/显示单个 chunk → 调 `services/indexing` 内部 API
...
- 批量操作：按 collection 批量 hide/unhide

### 5.4 检索策略配置（Retrieval Profile Management）

- Retrieval Profile CRUD：BM25 权重、vector 权重、rerank 模型、expansion 策略、pack budget、fail policy
- Profile 版本历史
- Collection 绑定：哪个 collection 用哪个 profile
- `services/admin` 是 Retrieval Profile 控制面 owner，负责创建、编辑、发布、停用和 collection 默认绑定
- `services/retrieval` 是 Retrieval Profile 运行时 owner，负责校验 profile 是否可执行、召回/融合/rerank/pack 语义、read-path cache key 和执行行为
- **生效发布流程**：
  1. admin 创建/编辑 profile（状态为 `draft`）
  2. 发布时，admin 调用 `services/retrieval` 的 `POST /internal/retrieval-profiles/validate`
  3. retrieval 返回 `canonical_config`、`profile_hash`、`validator_version`、`warnings`
  4. 若 `valid=false`，admin 拒绝发布，写 `ops_audit_log`（`after_state=rejected`），返回 409
  5. 若 `valid=true`，admin 将 `runtime_canonical_config`、`profile_hash`、`validator_version`、`warnings` 写入 profile 记录，状态变为 `published`
  6. 对已 `published` profile 再次发布时，自动创建新 version（旧版本 `retired`）
- published RetrievalProfile 采用不可变版本；修改已发布配置时只能生成新 version，并触发 profile hash / cache epoch 变化
- Retrieval cache 操作只允许通过 retrieval 内部 API 暴露的 purge/inspect 能力执行，admin 不直接操作 Redis key

### 5.5 Collection 管理

- `services/admin` 是 Collection Catalog 的事实 owner，负责 Collection CRUD、生命周期、默认策略绑定和基础访问策略
- Collection 默认绑定使用 versioned binding 记录；历史任务必须能回放到当时生效的 parser/retrieval/approval 绑定
- 配置 authority_level、access_policy
- 绑定 parser_profile、retrieval_profile
- 查看 collection 下的文档列表（published_documents）
- 批量操作：批量 reindex、批量 archive
...
- ApiKey Registry CRUD
- 分配 knowledge_scopes、roles、debug_permission、token_budget 限额
- 启用/禁用 key
- 创建、轮换、吊销、过期、scope 修改都由 `services/admin` 管理
- 查看 key 的调用频率和审计日志
- `services/admin` 是 API Key 控制面 owner，持有 key registry 和 key lifecycle
...
- Published Document 列表（支持按 collection、state 过滤）
- 查看生命周期状态、active index version、asset paths
- 操作：Archive / Retract / Reindex / Restore
- 下游代理：调 `services/intake-pipeline`（publishing domain）和 `services/indexing` 内部 API

#### 已实现的管理端点（2026-05-28）

```text
POST /admin/documents/{final_doc_id}/archive   # 归档已发布文档
POST /admin/documents/{final_doc_id}/retract   # 撤回已发布文档
POST /admin/documents/{final_doc_id}/reindex   # 触发重新索引
```
...
**reindex 流程**：
1. 鉴权
2. 从 intake-pipeline 查询当前 published document 关联的 parse_snapshot_id
...
- **Trace Timeline**（GraphQL）：trace + steps + artifacts + 跨服务关联（intake job / ticket / index build）
- **Job History**（GraphQL）：所有 job 类型的状态历史
- **Ops Audit Log**（GraphQL）：所有 admin 操作记录
- **Approval Audit**（GraphQL）：ticket + audit log 完整链路
- 聚合查询只能读取 owner 服务暴露的只读视图、投影表或报表 schema，不能用跨服务 join 的结果驱动控制操作

### 5.9 质量评测（Eval）

- **评测集管理**：question-answer pair 的 CRUD
- **Bad Case 管理**：录入 bad case，关联 query/chunk/expected_result，追踪修复状态
- **自动评测流水线触发**：对指定 collection + profile 跑自动评测
- **质量趋势看板**：NDCG、MRR、Recall@K 趋势
- 评测结果数据模型由 admin 本地维护

### 5.10 系统监控

- 服务健康状态（聚合各服务 `/health`）
- 队列深度（intake job 积压、index build 排队）
- LLM 调用成本（读 `llm_call_log` + `llm_cost_daily`）
- Collection 容量（chunk 数、存储量）
- 告警规则配置（job 失败率阈值、队列深度阈值、成本阈值）
...
| 功能 | 下游服务 | 需要的内部 API |
|------|---------|--------------|
| 审批覆盖 | `approval-service` | `POST /internal/tickets/{id}/override` |
| 解析预览 | `services/indexing` | `POST /internal/parse-previews`（已有）|
| 查询 ParseSnapshot | `services/indexing` | `GET /internal/parse-snapshots/{id}`（已有）|
| Chunk 隐藏/显示 | `services/indexing` | `PATCH /internal/chunks/{id}/visibility`（待补齐）|
| 索引版本激活/回滚 | `services/indexing` | `POST /internal/index-versions/{id}/activate`（已有）|
| 索引版本清理 | `services/indexing` | `POST /internal/index-versions/{id}/cleanup`（已有）|
| 发布文档 archive | `intake-pipeline` | `POST /internal/published-documents/{id}/archive`（待补齐）|
| 发布文档 retract | `intake-pipeline` | `POST /internal/published-documents/{id}/retract`（待补齐）|
...
| API Key projection 同步 | `services/access` | `POST /internal/api-key-projections/sync` 或 access 只读读取 admin projection（待补齐）|
...
| `RetrievalProfile` | `services/admin` | `services/retrieval` | CRUD、发布、停用、绑定 collection 默认值 | 无 | admin 本地表 + retrieval validate |
| `ApiKey` / external scope | `services/admin` | `services/access` | 创建、轮换、吊销、scope 管理 | 无 | admin 本地表 + access projection |
| `SourceFile` / `IntakeJob` | `intake-pipeline` | `intake-pipeline` | 查询、重放、运维 | 上传、看进度 | intake internal API |
| `ApprovalTicket` | `approval-service` | `approval-service` | override | pending review、decide | approval internal API |
| `AgentReviewArtifact` | `intake-pipeline` / `approval-service` | approval/workbench 展示 | 查询、审计 | 展示证据、辅助定位问题 | owner internal API |
| `PublishedDocument` | publishing domain in `intake-pipeline` | retrieval/indexing/access 读取投影 | archive/retract/reindex 入口 | 只读结果 | intake/publishing internal API |
| `ParseSnapshot` | `services/indexing` | `services/indexing` | 查询、诊断 | 预览、对比 | indexing internal API |
| `ChunkRevision` / indexed chunk replacement | `services/indexing` | indexing/retrieval | 运维兜底 | 发起人工修改 | indexing internal API |
...
- `display_name`
- `roles` JSON（`PLATFORM_ADMIN`, `KNOWLEDGE_ADMIN`）
- `clearance_level`
- `allowed_tenants` JSON
- `allowed_collections` JSON
...
### 8.4 `collection_profile_bindings`
- `binding_id` PK
...
### 8.5 `alert_rules`
- `rule_id` PK
- `rule_type`（`job_failure_rate`, `queue_depth`, `cost_threshold`）
- `threshold_value`
...
- `query`, `expected_evidence_id`, `actual_evidence_id`
- `collection_id`, `retrieval_profile_id`
...
- `name`, `description`
- `parser_id`（`naive`, `presentation`, `paper`, `qa`, ...）
- `parser_config` JSON
- `runtime_canonical_config` JSON nullable（indexing validate/canonicalize 后返回）
- `profile_hash` String — SHA-256 of deterministic canonical config JSON
- `validator_version` String — 运行时校验器版本（如 `indexing-v0.1.0`）
- `warnings` JSON — 校验产生的非致命警告
- `version`
- `state`（`draft`, `published`, `retired`）
- `created_by`, `created_at`, `updated_by`, `updated_at`

### 8.9 `retrieval_profiles`
- `retrieval_profile_id` PK
- `name`, `description`
- `profile_config` JSON（BM25/vector/rerank/expansion/pack/fail policy）
- `runtime_canonical_config` JSON nullable（retrieval validate/canonicalize 后返回）
- `profile_hash` String — SHA-256 of deterministic canonical config JSON
- `validator_version` String — 运行时校验器版本（如 `retrieval-v0.1.0`）
- `warnings` JSON — 校验产生的非致命警告
- `version`
- `state`（`draft`, `published`, `retired`）
- `created_by`, `created_at`, `updated_by`, `updated_at`

### 8.10 `api_key_registry`
- `api_key_id` PK
- `tenant_id`
- `key_hash`
- `display_name`
...
- `expires_at` nullable
- `created_by`, `created_at`, `updated_by`, `updated_at`, `last_rotated_at`

`token_budget_limit` 是 admin 控制面字段；如果 `services/access` 既有持久化或 projection 仍使用 `max_context_tokens` 内部列名，必须由 API Key projection adapter 显式映射，不能把旧列名暴露成 admin wire 字段。

## 9. GraphQL Schema 核心类型
...
POST /admin/auth/logout         # 作废 session
GET  /admin/auth/me             # 当前用户信息

# 运维控制
POST /admin/ops/jobs/{id}/retry
POST /admin/ops/jobs/{id}/cancel
POST /admin/ops/index-versions/{id}/rollback
POST /admin/ops/index-versions/{id}/cleanup
POST /admin/ops/intake-jobs/{id}/replay
POST /admin/ops/published-documents/{id}/archive
POST /admin/ops/published-documents/{id}/retract
POST /admin/ops/published-documents/{id}/restore
POST /admin/ops/tickets/{id}/override
POST /admin/ops/chunks/{id}/hide
POST /admin/ops/chunks/{id}/unhide
POST /admin/ops/collections/{id}/reindex

# 配置管理
GET    /admin/parser-profiles
POST   /admin/parser-profiles
GET    /admin/parser-profiles/{id}
PUT    /admin/parser-profiles/{id}
DELETE /admin/parser-profiles/{id}

GET    /admin/retrieval-profiles
POST   /admin/retrieval-profiles
GET    /admin/retrieval-profiles/{id}
PUT    /admin/retrieval-profiles/{id}
DELETE /admin/retrieval-profiles/{id}
...
# 评测
GET    /admin/eval/datasets
POST   /admin/eval/datasets
GET    /admin/eval/datasets/{id}
DELETE /admin/eval/datasets/{id}

GET    /admin/eval/bad-cases
POST   /admin/eval/bad-cases
GET    /admin/eval/bad-cases/{id}
PUT    /admin/eval/bad-cases/{id}
...
GET    /admin/metrics/llm-costs
GET    /admin/metrics/collection-capacity
GET    /admin/alert-rules
POST   /admin/alert-rules
PUT    /admin/alert-rules/{id}
...
- `parser_profiles`、`retrieval_profiles`、`api_key_registry`
- `ops_audit_log`
- 权限校验、tenant/collection 过滤
- API wire 使用 `token_budget_limit`；access projection 如需 `max_context_tokens`，在 adapter 中映射并覆盖测试

Phase 2：runtime owner 联调

- admin 发布 ParserProfile 前调用 indexing validate/canonicalize
- admin 发布 RetrievalProfile 前调用 retrieval validate/canonicalize
- API Key 创建/禁用/轮换后同步 access projection 或刷新 access key cache
- Collection 默认绑定变更后，相关服务能读到 projection 或 internal API
- profile publish / api key lifecycle / collection binding 变更都必须使用稳定 idempotency key，保证重试不产生双写

Phase 3：运维与观测

- approval override、published document archive/retract/reindex
...
- admin 保存 published profile 前，必须经过 indexing validate/canonicalize
...
- admin 保存 published profile 前，必须经过 retrieval validate/canonicalize
